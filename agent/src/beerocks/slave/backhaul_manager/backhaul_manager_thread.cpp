...
{
    std::string mac = network_utils::mac_to_string(iface_mac);
    if (network_utils::linux_iface_get_name(mac, iface_name)) {
        return true;
    }

    LOG(ERROR) << "Failed getting interface name for MAC address: " << mac;
    iface_name.clear();
                                                     ...
                                                     const std::string &src_mac)
{
    ...
    if (invalid_neighbor) {
        ...
        for (const auto &entry : neighbor_links_map) {
            sLinkInterface interface             = entry.first;
            std::vector<sLinkNeighbor> neighbors = entry.second;
            ...
            for (const auto &neighbor : neighbors) {
                ...
                LOG(TRACE) << "Getting link metrics for interface " << interface.iface_name
                           ...
                           << ") and neighbor " << network_utils::mac_to_string(neighbor.iface_mac);
                ...
                if (!collector->get_link_metrics(interface.iface_name, neighbor.iface_mac,
                    ...
                    LOG(ERROR) << "Unable to get link metrics for interface "
                               << interface.iface_name << " and neighbor "
                               << network_utils::mac_to_string(neighbor.iface_mac);
                    return false;
                }
                                                              ...
                                                              const std::string &src_mac)
{
    ...
    if (tlvSupportedRole) {
        ...
        LOG(ERROR) << "tlvSupportedRole missing - ignoring autoconfig response message";
        return true;
    }
