...
void monitor_stats::process()
{
    ...
    if (!requests_list.empty()) {
        ...
        auto elements_to_allocate =
            cmdu_tx.mtu_size_restriction_overflow_check(sizeof(beerocks_message::sStaStatsParams));
        ...
        for (auto it = mon_db->sta_begin(); it != mon_db->sta_end(); ++it) {
            ...
            if (elements_to_allocate == sta_count) {
                ...
                elements_to_allocate = cmdu_tx.mtu_size_restriction_overflow_check(
                    sizeof(beerocks_message::sStaStatsParams));
