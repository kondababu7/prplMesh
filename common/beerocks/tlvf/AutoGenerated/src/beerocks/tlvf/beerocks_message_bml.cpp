...
uint8_t& cACTION_BML_WIFI_CREDENTIALS_SET_REQUEST::authentication_type() {
    return (uint8_t&)(*m_authentication_type);
}

uint8_t& cACTION_BML_WIFI_CREDENTIALS_SET_REQUEST::encryption_type() {
    return (uint8_t&)(*m_encryption_type);
}
...
size_t cACTION_BML_WIFI_CREDENTIALS_SET_REQUEST::get_initial_size()
{
    ...
    class_size += sizeof(uint8_t); // authentication_type
    class_size += sizeof(uint8_t); // encryption_type
...
bool cACTION_BML_WIFI_CREDENTIALS_SET_REQUEST::init()
{
    ...
    m_authentication_type = (uint8_t*)m_buff_ptr__;
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    m_encryption_type = (uint8_t*)m_buff_ptr__;
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
